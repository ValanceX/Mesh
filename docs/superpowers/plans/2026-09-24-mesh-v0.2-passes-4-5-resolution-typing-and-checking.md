...
16. **`no-common-type` spans.** Equality: the whole comparison. Conditional: the whole conditional (the mistake is the combination). Array: the first element that has no common type with the ones before it, and the fact carries both types (`left` is the common type so far).
...
20. **Where values are checked.** A resolved prop's value (a quoted attribute is a `string`), each argument of a resolved command called with the right number of arguments (`$event` included), and each field value of an object literal checked against a record. An unknown prop's value, and the arguments of an unknown command or of one with the wrong arity, are only typed: the name or the arity is the one mistake. Content expressions (children) have no expected type (D15), since children aren't modelled.
...
22. **Arrays and conditionals are not checked contextually.** D15 types them by `join`, and pushing an expected type into them would be inference beyond D15 (a non-goal). A known consequence: `[{ a: 1 }, { a: 1, b: 2 }]` has no common type even where `list<{ a: number, b?: number }>` is expected. Shared named types and scope values are the intended way to pass lists of records; revisit with evidence.
...
| operand, condition, prop value, argument, field value | `is_assignable` | `TypeMismatch` | the value |
| `==`/`!=`, `? :`, array | `join` | `NoCommonType` | comparison / conditional / first misfit element |
...
10. **Suggestions are deterministic and close.** Pinned by `suggests_only_close_names`, `breaks_ties_by_candidate_order` and `measures_edit_distance_in_chars`.
11. **The examples check clean against the example manifest.** Pinned by `examples_check_clean_against_the_example_model`, which checks them for real from Task C3 on.
...
impl<'m> Walker<'m> {
    ...
    fn expect(&mut self, actual: Ty, span: Span, expectation: Expectation, expected: &Ty) {
        ...
        if !is_assignable(manifest, &actual, expected) {
            let is_optional = |ty: &Ty| matches!(relation::expand(manifest, ty), Ty::Optional(_));
            self.facts.push(Fact::TypeMismatch {
                expectation,
                expected: expected.clone(),
                possibly_absent: is_optional(&actual) && !is_optional(expected),
                actual,
                span,
            });
        }
...
+
+A value of type `any` fits everywhere a present value does. A value that may be absent (`T?`, including `any?`) fits only where absence is allowed, and then the message ends with `which may be absent`:
+
+```text
+error[type-mismatch]: the condition needs boolean, found boolean?, which may be absent
...
- [ ] **Step 2: Create the fixtures.** The outline's conformance cases (`conformance-*`), a pass fixture per D9 rule MPRX can reach (`assignable-rule-N-*`), a fail fixture per rule MPRX can break (`not-assignable-rule-N-*`), `$event` and argument checks, and one fail fixture per Pass 5b code:
...
```xml
<probe strings={[name, maybeName]} />
```

`examples/fixtures/check/fail/not-assignable-rule-7-lists.stderr`:

```text
error[type-mismatch]: the prop "strings" of component "probe" needs list<string>, found list<string?>
 --> fixtures/check/fail/not-assignable-rule-7-lists.mprx:1:17
  |
1 | <probe strings={[name, maybeName]} />
  |                 ^^^^^^^^^^^^^^^^^
```
...
- [ ] **Step 3: Run the tests to see them fail.**
  ...
  Expected: the build fails: `Expectation` has no `Prop`, `Argument` or `Field`, and `Fact` has no `UnknownField` or `MissingRequiredField`.
...
--- a/crates/mesh-analysis/src/lib.rs
+++ b/crates/mesh-analysis/src/lib.rs
@@ -88,6 +88,12 @@ pub enum Expectation {
     Operand(Operator),
     /// The condition of `c ? a : b`.
     Condition,
+    /// A prop's value.
...
- [ ] **Step 5: Check values in context.** In `crates/mesh-analysis/src/check.rs`: attribute values and handler arguments go through the new `check`, which hands object literals meeting a record to `object_against`:
...
+    /// arguments are only typed.
     fn command(
         &mut self,
         command: &str,
@@ -224,26 +240,140 @@ impl<'m> Walker<'m> {
         place: Place<'m>,
     ) {
         let commands = &self.template.component().commands;
-        match commands.get_key_value(command) {
...
+    /// its type is assignable to `expected`. An object literal where a
+    /// record is expected is checked field by field instead, with the
+    /// same relation. Returns the expression's type, if it has one.
+    fn check(
+        &mut self,
...
+        expectation: Expectation,
+    ) -> Option<Ty> {
+        if let Expression::Object { members, span } = expression {
+            // A literal is present, so where a record that may be absent
+            // is expected, the record itself is (rule 2).
...
+        ),
     ...
     Diagnostic {
         severity: Severity::Error,
@@ -204,6 +219,13 @@ fn expecting(expectation: &Expectation) -> String {
             format!("an operand of `{}`", binary(*operator))
         }
         Expectation::Condition => "the condition".to_string(),
...
@@ -697,6 +698,21 @@ What each place needs:
 |---|---|
 | `!x`, `x && y`, `x \|\| y`, the condition of `c ? a : b` | `boolean` |
 | `-x`, `x + y`, `x - y`, `x * y`, `x / y`, `x % y`, `x < y`, `x <= y`, `x > y`, `x >= y` | `number` |
...
@@ -742,6 +758,36 @@ error[duplicate-object-key]: the object already has the key "first"
...
+
 ...
 ## CLI errors
@@ -771,10 +817,4 @@ With `--model`, model errors come after all of those, in source order. Two error
...
- [ ] **Step 9: Run the gate.** Expected: 306 tests pass: 11 in `tests/checking.rs`. Every earlier test and fixture still passes unchanged, and `examples/users-page.mprx` and `examples/user-card.mprx` check clean against `examples/components.json`.
...
-These are errors with their own codes, listed under [Model errors](./diagnostics.md#model-errors), and a close match gets a `help` line. Whether prop values and command arguments have the right types isn't checked yet: that comes later in v0.2.
+6. **Values**: every prop value, command argument (`$event` included) and object literal field has a type that fits its declaration. An object literal given to a record type must have exactly the record's fields, every required one included.
+
...
@@ -13,6 +13,7 @@ All notable changes to MESH are recorded here. The project follows [Semantic Ver
 ...
 - **Checking against a manifest.** With `--model`, `mesh check` now checks the file as its component's template: every tag must be a declared component, every attribute one of its props (with every required prop supplied), every `on.` binding one of its events, every reference a name in the template's scope, and every command one of the template's commands, with the right number of arguments. Commands may appear only as the whole handler of an `on.` binding, and `$event` only in that command's arguments. Each mistake is its own error code: `unknown-component`, `unknown-prop`, `missing-required-prop`, `unknown-event`, `unknown-reference`, `unknown-command`, `command-arity-mismatch`, `command-outside-handler`, `handler-not-command`, `event-value-outside-handler`, `event-has-no-payload` and `unknown-special-value`. Without `--model`, nothing changes.
 - **Expression types.** With `--model`, every expression is typed. Reading a member that doesn't exist is `unknown-member`, and reading one through a value that may be absent is `possibly-absent-access`. Operators and conditions get the types they need or it's a `type-mismatch`: `+` adds numbers only, and nothing is converted implicitly. `==` and `!=`, the branches of `? :` and the elements of an array need a common type, or it's `no-common-type`. An object literal that repeats a key is `duplicate-object-key`. One mistake gets one diagnostic: an expression that has an error isn't checked any further.
+- **Value checking.** With `--model`, every prop value, command argument and `$event` is checked against its declared type, and a mismatch is a `type-mismatch`. A value that may be absent fits only where absence is allowed, and `null` is a value, not absence. An object literal given to a record type is checked field by field: a key the record doesn't declare is `unknown-field`, and a required field that's left out is `missing-required-field`. All of these checks use one compatibility rule.
 - **Suggestions.** A misspelled name gets a `= help: did you mean "user"?` line when the manifest declares a close match. In the Rust API, `Diagnostic` has a new `suggestions` field, and `Suggestion` holds a replacement and the span it replaces.
 - **Rust API.** A new crate, `mesh-analysis`, analyzes a template's Semantic IR against its manifest (`analyze`, `Analysis`), keeping what each name resolved to, each expression's type (`Ty`) and each problem found beside the IR, never in it. `is_assignable` and `join` are the type relations every check uses. `compile_with` uses it when given a template.
...
- [ ] **Step 4: Run the gate, and the MSRV check.** Expected: 306 tests pass.
...
Expected: 306 tests pass; the two examples print `no errors`; the last command exits 1 and prints `fixtures/check/fail/unknown-reference.stderr`, `help` line included.
