...
14. **The error marker is `Option<Ty>`'s `None`.** An expression with a fact about it, or about a part of it, has no type, and nothing that depends on its type is checked. So `usr.name + 1 > count` gets one `unknown-reference` and nothing else. The rules:
    - a member access whose object has an error, a conditional with an erroneous branch, an array with an erroneous element, and an object literal with an erroneous value have no type;
...
16. **`no-common-type` spans.** Equality: the whole comparison. Conditional: the whole conditional (the mistake is the combination). Array: the first element that has no common type with the ones before it, and the fact carries both types (`left` is the common type so far). From Pass 5b, an array literal or conditional where a type is expected is checked against it instead, and needs no common type (Decision 22).
17. **Duplicate object keys.** The first occurrence counts; each later one is `duplicate-object-key` at its key, and its value is still typed (its own mistakes are separate), but it isn't part of the literal's type and, in Pass 5b, isn't checked against a field.
...
22. **Array literals and conditionals are checked contextually, like object literals.** D15's `join` types them where nothing is expected (content, operands, equality). Where a type is expected, `check` pushes it in, as Decision 21 does for object literals. This is D9 checking against a declared type, not inference, which the outline's non-goal ("inference beyond D15's expression typing") rules out:
    ...
    - Only lists are pushed into arrays: an array where `any` is expected is typed by `join` and compared whole, so `[1, "a"]` is still `no-common-type` there.
...
| repeated object key | | `DuplicateObjectKey` | later key |
...
What is not looked inside: a misplaced command, a handler that isn't a command. What is typed but not compared: an unknown prop's value, an unknown or wrong-arity command's arguments, an object literal key that isn't a field, a repeated key's value, content expressions. What has no type: see Decision 14.
...
6. **One mistake, one diagnostic.** Pinned by `errors_do_not_cascade`, `errors_are_not_also_mismatches`, `a_handler_must_be_a_command_and_is_not_looked_inside`, `a_command_is_valid_only_as_a_whole_handler` and `one-mistake-one-diagnostic.mprx`.
...
9. **Object literals are exact, field by field.** Pinned by `an_object_literal_is_checked_field_by_field`, `nested_and_optional_records_are_checked_field_by_field`, `other_objects_are_compared_whole` and `a_repeated_key_is_reported_once`.
10. **An array literal or conditional given a type needs no common type.** `[{ name: "Ada", active: true }, { name: "Bo", active: false, avatar: "bo.png" }]` fits `list<User>`, and a wrong element or branch is reported at itself. Pinned by `an_array_literal_is_checked_element_by_element`, `each_branch_is_checked_against_the_expected_type`, `contextual-literals.mprx`, `element-type-mismatch.mprx` and `branch-type-mismatch.mprx`.
11. **"Which may be absent" only when absence is the reason.** `string?` where `number` is needed is a plain mismatch. Pinned by `unary_operators_need_and_give_their_type`.
12. **Suggestions are deterministic and close.** Pinned by `suggests_only_close_names`, `breaks_ties_by_candidate_order` and `measures_edit_distance_in_chars`.
13. **The examples check clean against the example manifest.** Pinned by `examples_check_clean_against_the_example_model`, which checks them for real from Task C3 on.
...
fn an_object_is_an_exact_record_of_required_fields() {
    ...
    let expression = "{ x: 1, y: name, x: usr }";
    assert_eq!(
        type_of(expression),
        (
            ...
            vec![
                Fact::DuplicateObjectKey {
                    key: "x".to_string(),
                    span: nth_span(expression, "x", 1),
                    first: span(expression, "x"),
                },
                Fact::UnknownReference {
                    name: "usr".to_string(),
...
```text
error[duplicate-object-key]: the object already has the key "first"
 --> fixtures/check/fail/duplicate-object-key.mprx:1:23
  |
...
+    /// is optional and `expected` isn't (outline D9, rule 2).
+    TypeMismatch {
+        expectation: Expectation,
+        expected: Ty,
+        actual: Ty,
...
+        span: Span,
+    },
+    /// An object literal repeats a key. Span: the repeated key; `first` is
+    /// the key it repeats.
+    DuplicateObjectKey {
+        key: String,
+        span: Span,
...
impl<'m> Walker<'m> {
    ...
    fn array(&mut self, elements: &[Expression], place: Place<'m>) -> Option<Ty> {
        let types: Vec<Option<Ty>> = elements
            ...
            .collect();
        let mut common = Ty::Nothing;
        for (element, ty) in elements.iter().zip(types) {
            common = self.common(common, ty?, element.span(), Combination::Elements)?;
        }
        Some(Ty::List(Box::new(common)))
    }

    /// An exact record with every key required, each typed by its value.
    /// A repeated key is reported, its value is still checked, and the
    /// first occurrence is the one that counts.
    fn object(&mut self, members: &[ObjectMember], place: Place<'m>) -> Option<Ty> {
        ...
        for member in members {
            ...
            if let Some(first) = first.get(member.key.as_str()) {
            ...
            first.insert(&member.key, member.key_span);
            match ty {
                Some(ty) => {
...
+            DiagnosticCode::DUPLICATE_OBJECT_KEY,
+            format!("the object already has the key {key:?}"),
+            Vec::new(),
+        ),
     };
     Diagnostic {
...
@@ -645,6 +646,102 @@ error[unknown-special-value]: unknown special value `$evnt`: the only one is `$e
...
+### `duplicate-object-key`
+
+An object literal has the same key twice. Points at the second one; the first is the one that counts.
+
+```text
+error[duplicate-object-key]: the object already has the key "first"
+ --> examples/fixtures/check/fail/duplicate-object-key.mprx:1:23
+  |
+1 | <text>{{ first: name, first: "Ada" }}</text>
+  |                       ^^^^^
+```
...
+
 ...
 ## CLI errors
@@ -676,7 +773,8 @@ Without `--model`, MESH checks only that a file is well-formed MPRX. Unknown com
...
@@ -12,8 +12,9 @@ All notable changes to MESH are recorded here. The project follows [Semantic Ver
 ...
 - **Rust API.** A new crate, `mesh-manifest`, loads and validates manifests (`load`, `Manifest`, `Template`). `mesh_compiler::compile_with` takes `CompileOptions`, which can carry a template; `compile` is unchanged.
 - **Checking against a manifest.** With `--model`, `mesh check` now checks the file as its component's template: every tag must be a declared component, every attribute one of its props (with every required prop supplied), every `on.` binding one of its events, every reference a name in the template's scope, and every command one of the template's commands, with the right number of arguments. Commands may appear only as the whole handler of an `on.` binding, and `$event` only in that command's arguments. Each mistake is its own error code: `unknown-component`, `unknown-prop`, `missing-required-prop`, `unknown-event`, `unknown-reference`, `unknown-command`, `command-arity-mismatch`, `command-outside-handler`, `handler-not-command`, `event-value-outside-handler`, `event-has-no-payload` and `unknown-special-value`. Without `--model`, nothing changes.
+- **Expression types.** With `--model`, every expression is typed. Reading a member that doesn't exist is `unknown-member`, and reading one through a value that may be absent is `possibly-absent-access`. Operators and conditions get the types they need or it's a `type-mismatch`: `+` adds numbers only, and nothing is converted implicitly. `==` and `!=`, the branches of `? :` and the elements of an array need a common type, or it's `no-common-type`. An object literal that repeats a key is `duplicate-object-key`. One mistake gets one diagnostic: an expression that has an error isn't checked any further.
 - **Suggestions.** A misspelled name gets a `= help: did you mean "user"?` line when the manifest declares a close match. In the Rust API, `Diagnostic` has a new `suggestions` field, and `Suggestion` holds a replacement and the span it replaces.
-- **Rust API.** A new crate, `mesh-analysis`, analyzes a template's Semantic IR against its manifest (`analyze`, `Analysis`), keeping what each name resolved to and each problem found beside the IR, never in it. `compile_with` uses it when given a template.
...
/// and the first occurrence is the one checked against the field.
#[test]
fn a_repeated_key_is_reported_once() {
    let source = r#"<card user={{ name: name, name: 1 }} />"#;
    assert_eq!(
        analyze(source).facts(),
        [Fact::DuplicateObjectKey {
            key: "name".to_string(),
            span: nth(source, "name", 2),
            first: nth(source, "name", 0),
        }]
    );
}
...
+    /// arguments are only typed.
     fn command(
         &mut self,
         command: &str,
@@ -224,26 +240,193 @@ impl<'m> Walker<'m> {
         place: Place<'m>,
     ) {
         let commands = &self.template.component().commands;
-        match commands.get_key_value(command) {
...
+            if field.required && !first.contains_key(name.as_str()) {
 ...
 **Fix:** compare or combine values of the same type.
@@ -742,6 +762,36 @@ error[duplicate-object-key]: the object already has the key "first"
...
+
 ...
 ## CLI errors
@@ -771,10 +821,4 @@ With `--model`, model errors come after all of those, in source order. Two error
