...
5. **Repeated key** (in a fixed-property object or a name map): `manifest-duplicate-key` at the later key; the later value is not traversed and never replaces the first. The **first** occurrence is the one that counts everywhere, `"version"` and `"kind"` included; nothing in the manifest is ever "last one wins". (`the_first_occurrence_of_a_repeated_property_counts`, `rejects_duplicate_keys_at_every_level`)
...
7. **`"kind"` discriminator:** the **first** `"kind"` member decides (rule 5). If the type object has no `"kind"` (`manifest-missing-property` at its `{`), a non-string first `"kind"` (`manifest-invalid-value`), or an unrecognised one (`manifest-unknown-kind`, `void` and `nothing` included), it gets that one diagnostic, and **none of its other properties are checked**: without a valid kind, no property's meaning is known, so unknown properties, repeated properties other than `"kind"`, and the kind-specific properties (`"element"`, `"fields"`, `"name"`, `"type"`) aren't checked or reported. **One exception:** every repeated `"kind"` key is still reported as `manifest-duplicate-key` at that key, so a later valid `"kind"` can't look as though it was used. With a valid first `"kind"`, a repeated `"kind"` (valid or not) is only a `manifest-duplicate-key`; its value is never used, and the other properties are checked by the first kind's rules. Validation outside the type object (its siblings, the rest of the document) continues either way. (`does_not_look_inside_a_rejected_value`, `a_valid_second_kind_does_not_replace_an_invalid_first_one`, `an_invalid_second_kind_does_not_replace_a_valid_first_one`)
...
Phases 1 and 2 report at most one diagnostic. The diagnostics of phases 3–5 are sorted by span start byte; diagnostics that start at the same byte keep the order they were emitted in: the traversal (phases 3–4) before phase 5, and emission order within each; and, for several missing properties of one object, the order the format lists them (for example `"type"` before `"required"`). A repeated `"kind"` always comes later in the source than the first, so its `manifest-duplicate-key` follows the first `"kind"`'s own error, if any. (`reports_every_problem_in_source_order`, `orders_diagnostics_at_the_same_position_deterministically`, `a_valid_second_kind_does_not_replace_an_invalid_first_one`)
...
7. **JSON's silent duplicate keys.** A repeated key at any of the eight levels is an error at the second occurrence, and the first occurrence is the one used. A repeated `"kind"` never replaces the first, whether the first is invalid (the rest of the type isn't checked) or valid (the rest is checked by its rules). Pinned in B2 (`rejects_duplicate_keys_at_every_level`, `the_first_occurrence_of_a_repeated_property_counts`, `a_valid_second_kind_does_not_replace_an_invalid_first_one`, `an_invalid_second_kind_does_not_replace_a_valid_first_one`).
...
Expected: PASS: 7 unit tests, 29 `load` tests, and 1 doctest (the `compile_fail` example on `Template`, which passes because building a `Template` by hand doesn't compile).
...
- **Contract coverage.** Every rule in "Manifest validation contract" names the test that pins it, and every test named there exists in B2's `load.rs` or `json.rs`. The contract describes the reference implementation's behaviour. One rule needed a validator change when its tests were added: rule 7's exception, reporting a repeated `"kind"` even when the first `"kind"` is invalid (`Validator::repeated_kinds`).
