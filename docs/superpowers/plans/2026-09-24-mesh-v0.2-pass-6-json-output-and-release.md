...
17. **The release date is 2026-09-24,** in three places: the CHANGELOG heading, the release notes and the README. If the release commit lands on another day, use that date in all three.
...
8. **The schema describes exactly what's printed.** It accepts every document `render_json` and the CLI print, and rejects an unknown severity, an unknown property and another version. Pinned by `the_schema_accepts_what_render_json_prints`, `the_schema_rejects_what_render_json_never_prints` and the corpus test.
...
| `docs/MPRX-SPEC.md` (modify) | F5 | §9 static semantics; renumbering. |
...
- [ ] **Step 1: The dependencies.** `serde` and `serde_json` are already in `Cargo.lock` (through `mesh-manifest`), and so is `serde_derive`; `jsonschema` is already `mesh-manifest`'s dev-dependency, with the same features:
  ...
  "$defs": {
    "diagnostic": {
      ...
      "properties": {
        ...
        "code": {
          "description": "The diagnostic's stable code. A code is never renamed or reused for a different meaning; new codes may be added.",
          "type": "string",
          "pattern": "^[a-z]+(-[a-z]+)*$"
        },
        "message": {
...
- [ ] **Step 3: Write the tests.** `crates/mesh-compiler/tests/json.rs`. They pin the exact document for a small case, the positions against the renderer's in every source the renderer treats specially, and the schema in both directions:
...
- [ ] **Step 7: Run the gate, and the MSRV check.** Expected: everything passes; `tests/json.rs` has 7 tests, and the workspace 320.
...
+enum Format {
+    /// rustc-style blocks on stderr, then `no errors` on stdout
+    Human,
...
- [ ] **Step 6: Run the gate, and the MSRV check.** Expected: everything passes; `tests/check.rs` has 17 tests, `tests/fixtures.rs` 11, and the workspace 327. Then look at it once by hand:
...
@@ -149,6 +217,14 @@ out=$(mesh check "$f" 2>&1 >/dev/null)
 [ -z "$out" ] || { printf '%s\n' "$out"; exit 1; }
 ```
...
- [ ] **Step 7: Run the gate, and the [link check](#the-link-check).** Expected: 328 tests pass, and `broken: 0`. If `jq` is installed, run Step 3's `jq` example on `examples/fixtures/fail/several-syntax-errors.mprx` and see its three lines.
...
- [ ] **Step 3: The language spec.** A new §9 (Decision 12), the pass table and status line, the "without a manifest" sentences in §4–§6, the renumbered out-of-scope list and history, and a revision-history entry:
...
+
+*Last updated 2026-09-24. v0.1 is released as v0.1.0 (see the [release notes](./releases/v0.1.md)). Pass 4 details are in `docs/superpowers/specs/2026-09-23-mesh-v0.1-pass-4-design.md`, and Pass 5 is outlined in `docs/superpowers/specs/2026-09-22-mesh-v0.1-pass-3-5-outline.md`. Any grammar or semantics work beyond v0.2 starts a new spec.*
...
| Name | Rule |
|---|---|
| component | an MPRX tag name: a letter or `_`, then letters, digits, `_` or `-` |
| type, prop, event, command, parameter, scope name, record field | an MPRX identifier: a letter or `_`, then letters, digits or `_`; and not `true`, `false` or `null` |
...
- [ ] **Step 7: Run the gate, and the [link check](#the-link-check).** Expected: 330 tests pass, and the link check prints `broken: 0`.
...
- Modify: `Cargo.toml`, `Cargo.lock`, `crates/mesh-lsp/Cargo.toml`, `CHANGELOG.md`, `README.md`, `docs/README.md`, `docs/guides/embedding-the-compiler.md`, `docs/guides/getting-started.md`, `docs/guides/writing-mprx.md`, `docs/manual/mesh-cli.md`
...
- **Syntax errors point at the mistake.** v0.1 reported every syntax error at `1:1`. v0.2 reports each one where it is, and common mistakes get their own message: a `<` in text, a hyphenated attribute name, an unclosed tag, a trailing comma, a single-brace object, a malformed `on.` binding.
...
+- **Tabs** in the source line are kept in the caret line's indentation, so carets line up in your terminal. Wide characters (CJK, emoji) may make the carets look shifted, because MESH doesn't measure display width.
 ...
 - **`= help:` lines** follow the carets when MESH has a suggestion, one line per suggestion. Most diagnostics have none. Today every suggestion is a similarly spelled name that the manifest declares.
 - **A leading UTF-8 byte-order mark** is skipped: it isn't echoed or counted, so line 1's columns match what your editor shows.
@@ -227,9 +227,9 @@ mesh check --format json "$f" |
...
`writing-mprx.md`'s event-binding paragraph said "v0.1 doesn't keep a registry of known events and doesn't check handler types", which stopped being true with `--model` in Pass 4.

- [ ] **Step 6: Run the gate, the MSRV check, and the [link check](#the-link-check).** Expected: 330 tests pass, `broken: 0`, and:
...
Expected: 330 tests pass; the grammar's 40 parses succeed; the two examples print `no errors`; the last command exits 1 and prints the one-line document shown in the CLI manual's "JSON output" section.
